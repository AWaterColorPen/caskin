...
func TestExecutorFrontend_Create(t *testing.T) {
	...
	assert.Equal(t, caskin.ErrCanOnlyAllowAtValidDomain, executor.CreateFrontend(frontend1, &example.Object{}))
	provider.Domain = stage.Options.GetSuperadminDomain()
	assert.Equal(t, caskin.ErrNoWritePermission, executor.CreateFrontend(frontend1, &example.Object{}))
...
func TestExecutorFrontend_Update(t *testing.T) {
	...
	assert.Equal(t, "backend$$sub_function", list1[2].ObjectCustomizedData.GetName())
}
...
func TestExecutorFrontend_Get(t *testing.T) {
	...
	assert.Equal(t, "frontend-root$$", list2[0].ObjectCustomizedData.GetName())
}
