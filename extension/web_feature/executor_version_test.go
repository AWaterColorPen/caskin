...
func TestExecutorVersion_SyncToUpdateAuth(t *testing.T) {
	...
	assert.Equal(t, caskin.ErrNoBackendPermission, executor.AuthBackendAPIEnforce(&web_feature.Backend{Path: "api/backend", Method: "POST"}))
	...
	provider.Domain = stage.Domain
	provider.User = stage.SubAdminUser
	assert.Equal(t, caskin.ErrNoBackendPermission, executor.AuthBackendAPIEnforce(&web_feature.Backend{Path: "api/backend", Method: "GET"}))
